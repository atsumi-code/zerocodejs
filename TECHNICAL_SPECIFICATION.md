...
- カテゴリタブ行右端の設定アイコンからポップオーバーで「選択した要素の前に追加する」「追加後に編集に移動」「挿入位置を表示」を切り替え可能
- 「追加後に編集に移動」をオンにすると、追加直後に編集モードへ切り替わり、追加したパーツの編集パネルが開く
- 「挿入位置を表示」ON 時、プレビュー上のパーツ間に挿入ボタン（24px・白背景の「＋」、選択中は緑背景）が表示される。`aria-label` で前後を区別。＋クリックは `insertBeforeActive` のみ更新し、チェック「選択した要素の前に追加する」とは独立
